...
class SunwinUltimateAI {
    ...
    // ----- Các phương thức dự đoán cụ thể (đã được rút gọn nhưng đầy đủ chức năng) -----
    markovPredict() {
...
let allHistorySessions = []; // Lưu tất cả phiên đã lấy được
...
async function fetchAndUpdate() {
    try {
        ...
        let sessions = Array.isArray(data) ? data : (data.data || [data]);
        if (!sessions.length) return;

        // Lấy phiên mới nhất (theo số phiên hoặc thời gian)
        const latest = sessions.reduce((a, b) => {
            let idA = a.phien || a.session || a.id || 0;
            let idB = b.phien || b.session || b.id || 0;
            return idA > idB ? a : b;
        }, sessions[0]);

        const sessionId = latest.phien || latest.session || latest.id;
        if (sessionId === lastSessionId) return;

        lastSessionId = sessionId;
        lastSessionData = latest;

        // Lưu vào lịch sử đầy đủ (nếu chưa có)
        if (!allHistorySessions.some(s => (s.phien || s.session || s.id) === sessionId)) {
            allHistorySessions.push(latest);
            // Giới hạn lịch sử 1000 phiên
            if (allHistorySessions.length > 1000) allHistorySessions.shift();
        }

        ai.addSession(latest);
        const pred = ai.predict();
        if (pred && !pred.wait) {
            currentPrediction = pred;
            console.log(`[${new Date().toISOString()}] Phiên ${sessionId} -> Dự đoán: ${pred.prediction} (${pred.confidence}%)`);
        } else {
            console.log(`[${new Date().toISOString()}] Chưa đủ dữ liệu (cần ít nhất 5 phiên)`);
        }
    } catch (err) {
...
// Endpoint chính: hiển thị dự đoán theo đúng định dạng
app.get('/', (req, res) => {
    if (!lastSessionData || !currentPrediction) {
        return res.status(503).send('Đang khởi tạo dữ liệu... Vui lòng đợi vài giây.');
    }

    const phien = lastSessionData.phien || lastSessionData.session || lastSessionData.id || 'N/A';
    const ketQua = lastSessionData.ket_qua || lastSessionData.result || 'N/A';
    const x1 = lastSessionData.xuc_xac_1 || lastSessionData.dice1 || 0;
    const x2 = lastSessionData.xuc_xac_2 || lastSessionData.dice2 || 0;
    const x3 = lastSessionData.xuc_xac_3 || lastSessionData.dice3 || 0;
    const xucXac = `${x1}-${x2}-${x3}`;
    const duDoan = currentPrediction.prediction;
    const doTinCay = currentPrediction.confidence + '%';
...
// Endpoint debug (tuỳ chọn)
app.get('/debug', (req, res) => {
    res.json({
        currentSession: lastSessionData,
        currentPrediction: currentPrediction,
        stats: ai.getStats(),
        historySize: allHistorySessions.length
    });
});

app.listen(PORT, () => {
    console.log(`Server đang chạy trên cổng ${PORT}`);
    console.log(`Truy cập http://localhost:${PORT} để xem dự đoán`);
    console.log(`Truy cập http://localhost:${PORT}/history để xem lịch sử phiên`);
});